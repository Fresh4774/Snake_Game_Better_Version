                ...
                switch(this.content) {
                    ...
                    case "Back":
                        ...
                        beginShape();
                            ...
                            vertex(this.x+this.size*0.25, this.y+this.size*0.25);
                            vertex(this.x+this.size*0.07, this.y+this.size*0.25);
                            vertex(this.x+this.size*0.07, this.y+this.size*0.12);
                            vertex(this.x-this.size*0.07, this.y+this.size*0.12);
                            vertex(this.x-this.size*0.07, this.y+this.size*0.25);
                            vertex(this.x-this.size*0.25, this.y+this.size*0.25);
                            vertex(this.x-this.size*0.25, this.y);
                            vertex(this.x, this.y-this.size*0.2);
                            vertex(this.x+this.size*0.25, this.y);
                        endShape();
                        noFill();
                        stroke(this.textColorHover);
    ...
    var Game = function(config) {
        ...
        this.messages = {
                cannibal: [
                    "You died loser",
                    "died again loser",
                    "wow how are you sooooooo bad at this game",
                    "you are worse then my dog and cat at the game LOSER",
                    "i knew it you are a big fat loser",
                    "LOSER LOSER"
                ],
                boundary: [
                    "loser loser",
                    "play again lol",
                    "wow, you died so fast",
                    "LOL",
                    "L take a L loser",
                    "cant you see loser"
                ],
                lazer: [
                    ...
                    "get wrecked",
                    "loser detected",
                    "you might win next time but i am still not sure"
                ],
                block: [
                    "blocks are cool",
                    "blocks are hard",
        ...
        this.setLevelGrid = function() {
            ...
            for(var row = 0; row < this.grid.length; row++) {
                for(var col = 0; col < this.grid[row].length; col++) {
                    ...
                    else if(this.grid[row][col] === "H" || this.grid[row][col] === "V") {
                        this.lazers.push(this.getLazer({
                            x: col * this.blockSize,
                            y: row * this.blockSize,
                            w: this.blockSize,
                            h: this.blockSize,
        ...
        this.checkCollision = function(){
            ...
            for(var i = this.blocks.length - 1; i >= 0; i--) {
                if(this.collision(this.blocks[i], head)) {
                    if(this.specialTimer > 0 && this.specialType === 0) {
                        var ex;
                        for(var n = 0; n < 10; n++) {
                            ex = this.getExplosion(
                                {
        ...
        this.checkEat = function(){
            ...
            for(var i = 0; i < this.foods.length; i++)
            {
                if(this.collision(this.foods[i], head)) {
                    ...
                    if(this.specialTimer > 0 && this.specialType === 2) {
                        this.score+= 20;
                    }
                    else {
                        this.score+= 10;
        ...
        this.howScreen = function() {
            ...
            pushStyle();
                ...
                textSize(16);
                text("i forgot to add instructions i am so sorry :)", width * 0.1, height * 0.15, width * 0.8, height);
            popStyle();
        ...
        this.nextScreen = function() {
            ...
            pushStyle();
                ...
                textSize(18);
                text("Current Score: " + (this.scoreTotal) + "\nBest Score: " + max(this.scoreBest, this.scoreTotal) + "\n\nPress C to generate a save code", width * 0.15, height * 0.25, width * 0.7, height);
            popStyle();
